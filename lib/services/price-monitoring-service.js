...
class PriceMonitoringService {
  constructor() {
    ...
    this.priceDropThresholdPercent = 5.0; // 5% drop required for notification
    this.priceIncreaseThresholdPercent = 10.0; // 10% increase required for notification
    this.minPriceChangeThreshold = 0.01; // Only track changes greater than 1 cent
  }
  ...
  async addMonitoredList(listUrl, customListName = null) {
    try {
      ...
      const existing = await database.get('SELECT id, is_active FROM monitored_lists WHERE list_url = ?', [listUrl]);
      ...
      if (existing) {
        if (existing.is_active) {
          ...
          await database.run('UPDATE monitored_lists SET is_active = 1, list_name = ?, updated_at = ? WHERE id = ?', [
            customListName || 'Reactivated List',
  ...
  async checkPriceChanges() {
    try {
      const activeLists = await database.all('SELECT * FROM monitored_lists WHERE is_active = 1');
      ...
      for (const list of activeLists) {
        ...
        await database.run('UPDATE monitored_lists SET updated_at = ? WHERE id = ?', [
          new Date().toISOString(),
          list.id,
        ]);
      }
  ...
  async checkListPrices(list) {
    ...
    try {
      ...
      const parts = await database.all('SELECT * FROM pc_parts WHERE list_id = ?', [list.id]);

      for (const part of parts) {
        // Find matching part in current data
        const currentPart = currentData.parts.find(
          (p) =>
        ...
        const percentChange = ((priceDifference / lastPrice.price) * 100);

        // Check if there's a significant price change
        if (Math.abs(priceDifference) > this.minPriceChangeThreshold) {
          // Insert new price record (always track all changes for historical accuracy)
          await database.run(
            'INSERT INTO price_history (part_id, price, merchant, availability, timestamp) VALUES (?, ?, ?, ?, ?)',
            [part.id, currentPart.price, currentPart.merchant, currentPart.availability, new Date().toISOString()],
          );

          // Only report price drops that meet the notification threshold
          const isPriceDrop = priceDifference < 0;
          const isPriceIncrease = priceDifference > 0;

          const shouldNotify =
            (isPriceDrop && Math.abs(percentChange) >= this.priceDropThresholdPercent) ||
            (isPriceIncrease && percentChange >= this.priceIncreaseThresholdPercent);

          if (shouldNotify) {
            // Record the price change for notification
            priceChanges.push({
  ...
  async removeMonitoredList(listId) {
    try {
      ...
      const result = await database.run('UPDATE monitored_lists SET is_active = 0, updated_at = ? WHERE id = ?', [
        new Date().toISOString(),
        listId,
      ]);
   ...
   * @param {number} priceDropPercent - Minimum percentage drop to trigger notification
   * @param {number} priceIncreasePercent - Minimum percentage increase to trigger notification
   * @returns {Object} - Result object
   */
  setNotificationThresholds(priceDropPercent, priceIncreasePercent) {
    try {
      if (priceDropPercent < 0 || priceDropPercent > 100) {
        return {
          success: false,
          error: 'Price drop percentage must be between 0 and 100',
        };
      }
      
      if (priceIncreasePercent < 0 || priceIncreasePercent > 100) {
        return {
          success: false,
          error: 'Price increase percentage must be between 0 and 100',
        };
      }

      this.priceDropThresholdPercent = priceDropPercent;
      this.priceIncreaseThresholdPercent = priceIncreasePercent;

      return {
        success: true,
        message: `Notification thresholds updated: ${priceDropPercent}% drops, ${priceIncreasePercent}% increases`,
      };
    } catch (error) {
      return {
        success: false,
  ...
  getNotificationThresholds() {
    return {
      priceDropThresholdPercent: this.priceDropThresholdPercent,
      priceIncreaseThresholdPercent: this.priceIncreaseThresholdPercent,
      minPriceChangeThreshold: this.minPriceChangeThreshold,
    };
  }
}
