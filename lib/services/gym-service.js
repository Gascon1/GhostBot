...
class GymService {
  ...
  async setWeeklyGoal(userId, weeklyGoal) {
    ...
    // Use SQLite's UPSERT (INSERT OR REPLACE) syntax
    await database.run(
      `
      INSERT INTO user_gym_goals (user_id, weekly_goal, created_at, updated_at)
      VALUES (?, ?, ?, ?)
    ...
    console.log('Weekly goal set successfully for user:', userId, 'goal:', weeklyGoal);
  ...
  async getUserWeeklyGoal(userId) {
    ...
    const weeklyGoal = goal ? goal.weekly_goal : 3; // Default to 3 times per week
    console.log('Retrieved weekly goal for user:', userId, 'goal:', weeklyGoal);
    return weeklyGoal;
  }
