 ...
 * @param {boolean} isGhostKing - Whether the monster is the ghost king
 * @returns {{flavor:string,art:string,end:string}} - The formatted ASCII art as a string.
 */
function determineAsciiArt(rollOutcome, member, odds, deadRole, xp, isGhostKing) {
  const isDead = member.roles.cache.has(deadRole.id);
  const hasDied = rollOutcome === 'died';
